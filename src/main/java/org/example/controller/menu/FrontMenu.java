...
public class FrontMenu {
    ...
    private User currentUser; // Remplacer l'ID par l'objet User
    ...
    private void loadContentIntoPane(String fxmlPath, String errorMessage) {
        try {
            ...
            // Try to set user if the controller supports it
            Object controller = loader.getController();
            if (controller != null) {
                try {
                    // Essayer d'abord setCurrentUser avec l'objet User
                    controller.getClass().getMethod("setCurrentUser", User.class).invoke(controller, currentUser);
                } catch (NoSuchMethodException e) {
                    // Si ça ne marche pas, essayer setCurrentUserId avec l'ID
                    try {
                        controller.getClass().getMethod("setCurrentUserId", int.class).invoke(controller, currentUser.getId());
                    } catch (NoSuchMethodException ex) {
                        // Le contrôleur ne supporte ni setCurrentUser ni setCurrentUserId
                        System.out.println("Controller does not support user injection: " + controller.getClass().getName());
                    }
                }
            }
